...
async def claim_task(c: Collector, until=0, key="") -> bool:
  ...
  if await is_task_claimed(c, True, key):
    return False
  return redis.setex(key, round(until or c.interval_sec * 1.2), state.proc_id) # 20% overtime buffer for long running tasks
...
async def free_task(c: Collector, key="") -> bool:
  key = key or claim_key(c)
  if not (await is_task_claimed(key)) or redis.get(key) != state.proc_id:
    return False
...
async def get_cache(name: str):
  return redis.get(cache_key(name))
